            ...
            <form action="<?= base_url('admin/struktur/anggota/update/' . $anggota['id_anggota']) ?>" method="post" class="space-y-6">
                        ...
                        <div class="relative">
                            <select id="level" 
                                ...
                                <?php foreach ($level as $l): ?>
                                    <option value="<?= $l['id_level'] ?>" <?= $l['id_level'] == $anggota['id_level'] ? 'selected' : '' ?>>
                                        <?= $l['nama_level'] ?>
                                    </option>
                                <?php endforeach ?>
                            </select>
                    ...
                    <div class="group">
                        ...
                        <div class="relative">
                            <select name="id_jabatan" id="jabatan" 
                                class="w-full appearance-none bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all cursor-pointer shadow-sm"
                                required>
                                <option value="">⏳ Loading Jabatan...</option>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400">
                ...
                <div class="group">
                    ...
                    <input type="text" name="nama" 
                        ...
                        value="<?= $anggota['nama'] ?>" placeholder="Masukkan nama tanpa gelar" required>
                </div>
...
<script>
    ...
    function loadJabatan(levelId) {
        ...
        jabatanSelect.parentElement.classList.add('opacity-50');
        ...
        fetch(`<?= base_url('admin/struktur/jabatan-by-level') ?>/${levelId}`)
            ...
            .then(data => {
                ...
                data.forEach(j => {
                    const isSelected = j.id_jabatan == selectedJabatan ? 'selected' : '';
                    option += `<option value="${j.id_jabatan}" ${isSelected}>${j.nama_jabatan}</option>`;
                });
                jabatanSelect.innerHTML = option;
                jabatanSelect.parentElement.classList.remove('opacity-50');
            })
            .catch(() => {
    ...
    // Listener untuk perubahan Level
